// app.js (全機能統合・実行順序修正・省略なし最終版)

document.addEventListener('DOMContentLoaded', () => {
    
    const db = new Dexie('OnePieceCardDB_v19'); // DB名を変更してリセット
    db.version(1).stores({
    ...
    async function initializeApp() {
        ...
        try {
            ...
            DOM.appContainer.innerHTML = `<main class="app-content"><div id="status-message">エラー: ${error.message}。</div></main>`;
        }
    }
    ...
    async function syncData(lastModified) {
        DOM.appContainer.innerHTML = `<main class="app-content"><div id="status-message">カードデータを更新中...</div></main>`;
    ...
    async function renderDeckListScreen() {
        DOM.appContainer.innerHTML = `<main class="app-content"><div style="padding:1rem"><h2>デッキ一覧</h2><p>デッキ機能は実装中です</p></div></main>`;
    }

    async function displayCards() {
        const cardList = document.getElementById('card-list');
        const statusMsg = document.getElementById('status-message');
        if (!cardList || !statusMsg) return;
        try {
            if (state.allCards.length === 0) {
                statusMsg.textContent = 'カードデータがありません。'; statusMsg.style.display = 'block'; return;
            }
            let filtered = [...state.allCards];
            if (Object.keys(state.filters).length > 0) {
                filtered = state.allCards.filter(card => Object.keys(state.filters).every(key => {
    ...
    function openLightbox(src) {
        const lightboxHtml = `<div id="lightbox-modal" class="modal-overlay"><span id="close-lightbox-btn" class="close-btn lightbox-close">&times;</span><img class="lightbox-content" src="${src}"></div>`;
        DOM.modalContainer.innerHTML = lightboxHtml;
        const modal = document.getElementById('lightbox-modal');
        modal.addEventListener('click', (e) => { if(e.target === modal) DOM.modalContainer.innerHTML = ''; });
        document.getElementById('close-lightbox-btn').addEventListener('click', () => DOM.modalContainer.innerHTML = '');
    }

    async function openFilterModal() {
        const filterModalHtml = `<div id="filter-modal" class="modal-overlay"><div class="modal-content"><div class="modal-header"><h2>フィルタ</h2><button id="close-filter-btn" class="close-btn"><i class="fa-solid fa-xmark"></i></button></div><div id="filter-options" class="modal-body"></div><div class="modal-footer"><button id="clear-filter-btn">クリア</button><button id="apply-filter-btn">適用</button></div></div></div>`;
        DOM.modalContainer.innerHTML = filterModalHtml;
        await setupFilters();
        document.getElementById('close-filter-btn').addEventListener('click', () => DOM.modalContainer.innerHTML = '');
        ...
        document.getElementById('filter-modal').addEventListener('click', (e) => { if (e.target.id === 'filter-modal') DOM.modalContainer.innerHTML = ''; });
    }
    ...
    async function setupFilters() {
        ...
        const uniqueValues = { color: new Set(), cardType: new Set(), attribute: new Set(), rarity: new Set(), series: new Map() };
        state.allCards.forEach(card => {
            ...
            if(card.seriesCode && !card.cardNumber.startsWith('P-')) { uniqueValues.series.set(card.seriesCode, card.seriesTitle); }
        });
        const raritiesWithoutSP = Array.from(uniqueValues.rarity).filter(r => r !== 'SP');
        const createButtons = (values, key) => values.sort((a,b) => a.localeCompare(b, undefined, {numeric: true})).map(val => `<button class="filter-option-btn ${state.filters[key]?.includes(val) ? 'active' : ''}" data-filter="${key}" data-value="${val}">${val}</button>`).join('');
        const seriesOptions = Array.from(uniqueValues.series.entries()).sort((a, b) => a[0].localeCompare(b[0], undefined, {numeric: true})).map(([code, title]) => `<option value="${code}" ${state.filters.seriesCode?.includes(code) ? 'selected' : ''}>${code} ${title}</option>`).join('');
        filterOptions.innerHTML = `<div class="filter-section"><h3>TYPE</h3><div class="filter-buttons">${createButtons(Array.from(uniqueValues.cardType), 'cardType')}</div></div><div class="filter-section"><h3>COLOR</h3><div class="filter-buttons color-filter">${createButtons(Array.from(uniqueValues.color), 'color')}</div></div><div class="filter-section"><h3>RARITY</h3><div class="filter-buttons">${createButtons(raritiesWithoutSP, 'rarity')}</div></div><div class="filter-section"><h3>SERIESフィルタ</h3><select class="series-select" id="filter-series"><option value="all">SERIESを選択</option>${seriesOptions}</select></div><div class="filter-section"><h3>ATTRIBUTES</h3><div class="filter-buttons">${createButtons(Array.from(uniqueValues.attribute), 'attribute')}</div></div><div class="filter-section"><h3>COST</h3><div class="filter-buttons">${[...Array(11).keys()].map(i => `<button class="filter-option-btn ${state.filters.costLifeValue?.includes(String(i)) ? 'active' : ''}" data-filter="costLifeValue" data-value="${i}">${i}</button>`).join('')}</div></div>`;
        filterOptions.querySelectorAll('.filter-option-btn').forEach(btn => btn.addEventListener('click', () => btn.classList.toggle('active')));
    }

    function openSettingsModal() {
        const settingsHtml = `<div id="settings-modal" class="modal-overlay"><div class="modal-content"><h2>設定</h2><div class="settings-options"><button id="cache-images-btn">全画像キャッシュ</button><button id="clear-cache-btn">全キャッシュ削除</button></div><button id="close-settings-btn" class="close-btn">閉じる</button></div></div>`;
        DOM.modalContainer.innerHTML = settingsHtml;
        ...
        document.getElementById('close-settings-btn').addEventListener('click', () => DOM.modalContainer.innerHTML = '');
        document.getElementById('settings-modal').addEventListener('click', (e) => { if(e.target.id === 'settings-modal') DOM.modalContainer.innerHTML = ''; });
    }

    async function cacheAllImages() {
        if (!('serviceWorker' in navigator) || !navigator.serviceWorker.controller) { alert('Service Workerが有効ではありません。'); return; }
        ...
        btn.disabled = true; btn.textContent = 'キャッシュ中...';
        ...
        try {
            ...
            navigator.serviceWorker.onmessage = (event) => {
                ...
                if (event.data.type === 'CACHE_COMPLETE') {
                    ...
                    if (progressText) progressText.textContent = `キャッシュ完了！`;
                    setTimeout(() => { if (progressBarContainer) progressBarContainer.style.display = 'none'; }, 2000);
                    btn.disabled = false; btn.textContent = '全画像キャッシュ';
                }
            };
        } catch (err) {
    ...
    function setupGlobalEventListeners() {
        DOM.navItems.forEach(item => {
            item.addEventListener('click', (e) => { e.preventDefault(); const screen = e.currentTarget.dataset.screen; if(screen && screen !== state.currentScreen) navigateTo(screen); });
        });
